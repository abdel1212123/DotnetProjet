...
namespace DotnetProjet.Structure;
...
public class DaoImpl : IDao
{
    ...
    public bool SessionExists(int sessionId) => _context.Sessions.Any(s => s.Id == sessionId);

    public bool DejaInscrit(int userId, int sessionId)
        => _context.Users.Where(u => u.Id == userId).SelectMany(u => u.Sessions).Any(s => s.Id == sessionId);

    public void Inscrire(int userId, int sessionId)
    {
        var user = _context.Users.Include(u => u.Sessions).First(u => u.Id == userId);
    ...
    {
        var user = _context.Users.Include(u => u.Sessions).ThenInclude(s => s.Formation).FirstOrDefault(u => u.Id == userId);
        return user?.Sessions.ToList() ?? new List<Session>();
    }

    public List<Formateur> GetAllFormateurs() => _context.Formateurs.ToList();
    public Formateur? GetFormateurById(int id) => _context.Formateurs.Find(id);
    ...
    public List<Formation> GetAllFormations()
        => _context.Formations.Include(f => f.Formateur).Include(f => f.Categorie).ToList();

    public Formation? GetFormationById(int id)
        => _context.Formations.Include(f => f.Formateur).Include(f => f.Categorie).Include(f => f.Sessions).FirstOrDefault(f => f.Id == id);
    ...
    public List<Session> GetAllSessions() => _context.Sessions.Include(s => s.Formation).ToList();
    public Session? GetSessionById(int id) => _context.Sessions.Include(s => s.Formation).FirstOrDefault(s => s.Id == id);
    ...
    public List<Module> GetAllModules() => _context.Modules.Include(m => m.Contenus).ToList();
    public Module? GetModuleById(int id) => _context.Modules.Include(m => m.Contenus).FirstOrDefault(m => m.Id == id);
    ...
    public List<Contenu> GetAllContenus() => _context.Contenus.ToList();
    public Contenu? GetContenuById(int id) => _context.Contenus.Find(id);
